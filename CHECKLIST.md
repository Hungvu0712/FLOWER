...
- [x] [TEST-01] Test luồng phiên phía client: component test Nav khi phiên chết; E2E xoá cookie `access_token` rồi bấm Link *(26/09/2026 — đã có sẵn từ lúc merge nhánh `review` (commit `676ae9e`), chỉ chưa tick: `frontend/tests/components/session-expiry.test.tsx` (10 test, gồm đúng "Nav đang hiện tên user → refresh token chết → Nav chuyển về link Đăng nhập") + `frontend/e2e/session-expiry.spec.ts` (3 test, `clearCookies({name:'access_token'})` rồi bấm Link — đúng kỹ thuật review yêu cầu, tránh `page.goto()` vì không tái hiện được bug FE-08). Chạy lại xác nhận thật: cả 10 component test và cả 3 E2E test (dùng backend + browser thật, không mock) đều xanh)*

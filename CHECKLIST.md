...
- [x] **Uptime monitor trỏ vào `/health`** *(14/09/2026 — UptimeRobot free (50 monitor, check mỗi 5 phút), monitor `HTTP(s)` trỏ `https://api.thuymaiflower.click/health`, cảnh báo qua email. Xác nhận THẬT: dashboard báo "Up", uptime 100%. Chưa thêm monitor riêng cho frontend (`thuymaiflower.click`) — có thể thêm sau, không bắt buộc theo đúng nội dung mục này)*

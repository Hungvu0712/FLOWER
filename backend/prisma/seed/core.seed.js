...
// Domain (role/permission riêng nghiệp vụ) nằm ở prisma/seed/domain.seed.js.
// Xem DATABASE.md §4, ARCHITECTURE.md §2.3.
...
const CORE_PERMISSIONS = [
  { code: 'users.manage', groupName: 'users', isRestricted: true, description: 'Block/unblock, reset password, đổi role user (chỉ super_admin)' },
  { code: 'settings.manage', groupName: 'settings', isRestricted: true, description: 'Cấu hình hệ thống, bật/tắt phương thức đăng nhập (chỉ super_admin)' },
  { code: 'roles.manage', groupName: 'roles', isRestricted: true, description: 'Tạo/sửa/xoá role tuỳ ý, gán permission cho role (chỉ super_admin)' },
...
const CORE_ROLES = [
  ...
  { code: 'member', name: 'Member', isSystem: true, permissionCodes: [] },
];
...
async function main() {
  ...
  for (const p of CORE_PERMISSIONS) {
    await prisma.permission.upsert({
      ...
      update: { groupName: p.groupName, isRestricted: p.isRestricted, description: p.description, isSystem: true },
    });
  }
  ...
  if (!existing) {
    ...
    console.log(`  -> Đã tạo super_admin: ${email} / ${password} (ĐỔI MẬT KHẨU NGAY sau lần đăng nhập đầu)`);
  } else {

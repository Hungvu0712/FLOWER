...
function mockAllowedPermissions(allowedIds: number[]) {
  db.permission.findMany.mockImplementation(async ({ where }: { where: { id: { in: number[] } } }) =>
...
describe("create — chốt chặn shadow super_admin", () => {
  it("LỌC BỎ permission is_restricted kể cả khi client cố tình gửi kèm", async () => {
    ...
    const created = db.role.create.mock.calls[0]![0].data.permissions.create as { permissionId: number }[];
  ...
  it("ghi audit log role.create", async () => {
    ...
    expect(auditLog.record).toHaveBeenCalledWith(
      expect.objectContaining({ actorId: ACTOR, action: "role.create", entityId: 10, ipAddress: "1.2.3.4" }),
    );
  });
});
...
describe("update", () => {
  ...
  it("404 khi role không tồn tại", async () => {
    ...
    await expect(service.update(ACTOR, 999, {} as never)).rejects.toMatchObject({ statusCode: 404 });
  });
});
...
describe("remove", () => {
  ...
  it("409 ROLE_IN_USE khi còn user đang gán role (tránh user mất hết quyền đột ngột)", async () => {
    ...
    await expect(service.remove(ACTOR, 10)).rejects.toMatchObject({ statusCode: 409, code: "ROLE_IN_USE" });
  });
  ...
  it("xoá được Custom Role không còn ai dùng", async () => {
    ...
    expect(auditLog.record).toHaveBeenCalledWith(expect.objectContaining({ action: "role.delete" }));
  });
});

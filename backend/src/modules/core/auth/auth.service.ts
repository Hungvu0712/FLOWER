...
function assertActive(user: User | null): asserts user is User {
  ...
  if (user.status === "blocked")
    throw new AppError("Tài khoản đã bị khoá", 403, "ACCOUNT_BLOCKED");
  if (user.deletedAt)
    throw new AppError("Tài khoản không tồn tại", 401, "INVALID_CREDENTIALS");
}

async function assertMethodEnabled(
  method: string,
  label: string,
): Promise<void> {
  const enabled = await repo.isLoginMethodEnabled(method);
  if (!enabled)
    throw new AppError(`${label} hiện đang tắt`, 403, "LOGIN_METHOD_DISABLED");
}
...
export async function requestMagicLink(input: {
  ...
  if (user && user.status !== "blocked" && !user.deletedAt) {
    ...
    const expiresAt = new Date(
      Date.now() + env.magicLink.ttlMinutes * 60 * 1000,
    );
...
export async function refreshSession(
  ...
  if (!refreshToken)
    throw new AppError("Thiếu refresh token", 401, "UNAUTHENTICATED");

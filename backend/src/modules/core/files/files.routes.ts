...
filesRouter.get('/', authorize('files.manage'), validate({ query: listFilesQuerySchema }), controller.list);
filesRouter.delete('/:id', authorize('files.manage'), validate({ params: fileIdParamSchema }), controller.remove);

...
export async function getPresignedUploadUrl({ originalName, mimeType, sizeBytes, folderId }: PresignInput) {
...
export async function listFiles({ folderId, page, limit }: ListFilesQuery) {
  ...
  const [items, total] = await Promise.all([
    prisma.file.findMany({ where, orderBy: { createdAt: 'desc' }, skip: (page - 1) * limit, take: limit }),
...
export async function setEntityFile({ fileId, entityType, entityId }: EntityFileInput): Promise<void> {
...
export async function addFileUsage({ fileId, entityType, entityId }: EntityFileInput): Promise<void> {

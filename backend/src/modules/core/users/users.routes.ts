...
usersRouter.post('/change-password', validate({ body: changePasswordSchema }), controller.changePassword);
usersRouter.get('/sessions', controller.listSessions);
usersRouter.delete('/sessions/:id', validate({ params: sessionIdParamSchema }), controller.revokeSession);

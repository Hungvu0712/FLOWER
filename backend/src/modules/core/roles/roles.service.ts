...
async function stripRestrictedPermissionIds(permissionIds: number[] | undefined): Promise<number[]> {
...
export async function create(actorId: string, input: CreateRoleInput, ipAddress?: string) {
  ...
  await auditLog.record({ actorId, action: 'role.create', entityType: 'role', entityId: role.id, after: role, ...(ipAddress && { ipAddress }) });
  return role;
}

export async function update(actorId: string, id: number, input: UpdateRoleInput, ipAddress?: string) {
  ...
  await auditLog.record({ actorId, action: 'role.update', entityType: 'role', entityId: id, before, after: updated, ...(ipAddress && { ipAddress }) });
  return updated;
}
...
export async function remove(actorId: string, id: number, ipAddress?: string): Promise<void> {
  ...
  await auditLog.record({ actorId, action: 'role.delete', entityType: 'role', entityId: id, before: role, ...(ipAddress && { ipAddress }) });
}

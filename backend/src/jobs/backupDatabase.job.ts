...
// ARCHITECTURE.md §7. Chạy 2 ngày/lần (jobs/index.ts).
...
// Tự động xoá backup cũ hơn 30 ngày — xem SECURITY.md §5.
export async function cleanupOldBackups(): Promise<void> {
  ...
  for (const obj of expired) {
    if (obj.Key) await r2Client.send(new DeleteObjectCommand({ Bucket: env.r2.bucket, Key: obj.Key }));
  }

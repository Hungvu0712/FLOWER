...
describe('loginSchema', () => {
  ...
  it('báo lỗi tiếng Việt khi email sai định dạng', () => {
    expect(firstError(loginSchema.safeParse({ email: 'sai', password: 'x' }))).toBe(
      'Email không hợp lệ',
    );
  });
...
describe('registerSchema — phải khớp với backend auth.validation.ts', () => {
  it('yêu cầu mật khẩu tối thiểu 8 ký tự, cùng thông điệp với backend', () => {
    expect(
      firstError(
        registerSchema.safeParse({ fullName: 'A', email: 'a@example.com', password: '1234567' }),
      ),
    ).toBe('Mật khẩu tối thiểu 8 ký tự');
    expect(
      registerSchema.safeParse({ fullName: 'A', email: 'a@example.com', password: '12345678' })
        .success,
    ).toBe(true);
  });
  ...
  it('bắt buộc họ tên', () => {
    expect(
      firstError(
        registerSchema.safeParse({ fullName: '', email: 'a@example.com', password: '12345678' }),
      ),
    ).toBe('Vui lòng nhập họ tên');
  });
});

describe('resetPasswordSchema', () => {
  it('áp cùng rule 8 ký tự như lúc đăng ký', () => {
    expect(firstError(resetPasswordSchema.safeParse({ newPassword: '1234567' }))).toBe(

...
// TODO: thay bằng thông tin THẬT của cửa hàng — xem cùng ghi chú ở components/storefront/ProductCard.tsx.
const HOTLINE = '0900000000';
const HOTLINE_DISPLAY = '0900 000 000';
const ADDRESS = '123 Đường Hoa, Quận 1, TP. Hồ Chí Minh';
const OPEN_HOURS = '07:00 – 21:00 tất cả các ngày';
const ZALO_LINK = `https://zalo.me/${HOTLINE}`;
const ZALO_DISPLAY = `zalo.me/${HOTLINE}`;

type FormState = { name: string; phone: string; occasion: string; message: string };
const emptyForm: FormState = { name: '', phone: '', occasion: '', message: '' };

const PhoneIcon = (
  <svg className="h-full w-full" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.9">
...
function InfoRow({ label, value, href }: { label: string; value: string; href?: string }) {
  return (
    <div className="border-b border-border-soft py-5 first:pt-0 last:border-none last:pb-0">
      ...
      {href ? (
        <a href={href} target={href.startsWith('http') ? '_blank' : undefined} rel="noopener noreferrer" className="mt-1 block text-lg font-semibold text-rose hover:text-rose-dark">
          {value}
        </a>
      ) : (
...
function PillLink({ icon, label, value, href }: { icon: React.ReactNode; label: string; value: string; href: string }) {
  return (
    <a
      href={href}
    ...
    >
      <span className="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-full bg-rose-light p-2 text-rose-dark">
        {icon}
      </span>
      <span>
        <span className="block text-[11px] font-semibold tracking-wide text-ink-muted uppercase">{label}</span>
  ...
  return (
    <div>
      ...
      <section className="bg-rose-light px-8 py-16 text-center lg:px-16">
        ...
        <h1 className="font-display text-4xl font-semibold text-ink lg:text-5xl">Đặt hoa nhanh, chỉ một cuộc gọi</h1>
        <p className="mx-auto mt-4 max-w-xl text-ink-muted">
          Gọi điện hoặc nhắn Zalo để được tư vấn nhanh nhất, mở cửa {OPEN_HOURS}. Ngoài giờ vẫn có thể
          để lại lời nhắn, chúng tôi phản hồi ngay khi mở cửa lại.
        </p>
      </section>
      ...
      <section className="px-8 py-16 lg:px-16">
        <div className="grid gap-12 lg:grid-cols-2">
          <div>
            <h2 className="mb-2 font-display text-2xl font-semibold text-ink">Thông tin cửa hàng</h2>
            <div className="mt-4">
            ...
            {sent ? (
              ...
              <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <div>
                    <label className="mb-1.5 block text-xs font-medium text-ink-muted">Họ và tên</label>
                    <input
                      value={form.name}
                ...
                <div>
                  <label className="mb-1.5 block text-xs font-medium text-ink-muted">Dịp cần hoa (tuỳ chọn)</label>
                  <input
                ...
                <div>
                  <label className="mb-1.5 block text-xs font-medium text-ink-muted">Lời nhắn</label>
                  <textarea
                    rows={4}
                ...
                <Button type="submit" loading={submitContact.isPending} disabled={!form.name || !form.phone || !form.message}>
                  Gửi yêu cầu
                </Button>
              </form>
            )}
        ...
        <div className="flex flex-col gap-8 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-md text-sm text-white/70">
              Gọi hoặc nhắn Zalo cho chúng tôi — tư vấn mẫu phù hợp và báo giá ngay trong vài phút, hỗ
              trợ cả đơn gấp cần giao trong ngày.
            </p>
          </div>
          <div className="flex flex-col gap-3 sm:flex-row">
            <PillLink icon={PhoneIcon} label="Gọi đặt hoa ngay" value={HOTLINE_DISPLAY} href={`tel:${HOTLINE}`} />

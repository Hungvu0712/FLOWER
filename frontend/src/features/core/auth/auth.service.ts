...
export const authService = {
  getLoginMethods: () => api.get<{ data: LoginMethod[] }>('/api/v1/auth/login-methods').then((r) => r.data.data),

  login: (input: LoginInput) => api.post<{ data: { user: AuthUser } }>('/api/v1/auth/login', input).then((r) => r.data.data),

  register: (input: RegisterInput) =>
    api.post<{ data: { user: AuthUser } }>('/api/v1/auth/register', input).then((r) => r.data.data),

  requestMagicLink: (input: MagicLinkRequestInput) => api.post('/api/v1/auth/magic-link/request', input),

  verifyMagicLink: (token: string) =>
    api.post<{ data: { user: AuthUser } }>('/api/v1/auth/magic-link/verify', { token }).then((r) => r.data.data),

  loginWithGoogle: (idToken: string) =>
    api.post<{ data: { user: AuthUser } }>('/api/v1/auth/google', { idToken }).then((r) => r.data.data),

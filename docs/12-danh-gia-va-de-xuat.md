...
| `BE-21` | `authLimiter` (20 request/15 phút/IP, dùng chung cho `/register`+`/login`+`/google`+`/forgot-password`+`/reset-password`) làm cả bộ E2E thật (32 test, `workers:1`, `retries:2` trên CI) fail dây chuyền — hàng chục request auth liên tục từ 1 IP (runner) vượt ngưỡng giữa suite, phát hiện lúc chạy full E2E lần đầu trên CI | Thêm cờ `DISABLE_RATE_LIMIT` (mặc định `false`) — `skip: () => env.disableRateLimit` ở cả 3 limiter trong `auth.routes.ts`, chỉ bật `true` ở job `e2e` trong `.github/workflows/ci.yml`, không đổi hành vi dev/staging/production | 30 phút | ✅ *(14/09 — xem docs/09 §2 + `.env.example`)* |
...
| `FE-10` | 7 dòng `console.log` DEBUG lọt vào commit thật lúc điều tra bug đăng nhập (`proxy.ts`, `lib/redirect.ts`, `login/page.tsx`, xem docs/12 FE-08/FE-09) — không có gì chặn tái diễn (review-source CODE-01) | Thêm rule ESLint `no-console` (cho phép `warn`/`error` — `error.tsx`/`global-error.tsx` dùng đúng chỗ) | 15 phút | ✅ *(26/09 — `frontend/eslint.config.mjs`, mức `"error"` (không phải `"warn"`) để CI thật sự chặn, không chỉ nhắc; xác nhận bằng file thử `console.log` tạm — lint báo lỗi đúng như kỳ vọng, rồi xoá file thử)* |

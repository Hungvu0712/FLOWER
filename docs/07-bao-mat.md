...
| Refresh token bị lộ | ✅ Refresh token **rotation**: mỗi lần dùng để cấp access token mới thì phát hành refresh token mới, thu hồi token cũ (set `sessions.revoked_at`); chỉ lưu `refresh_token_hash`, không lưu token thô. **Reuse detection**: token đã **xoay vòng** (`sessions.rotated_at`) mà vẫn được gửi lên lại → thu hồi TOÀN BỘ session của user + ghi audit log (`auth.refresh_reuse_detected`) + gửi email cảnh báo — xem [12 · BE-03](12-danh-gia-va-de-xuat.md). Token bị thu hồi vì lý do **hợp lệ** (đăng xuất thiết bị, đổi mật khẩu, superadmin khoá/reset) mà trình duyệt cũ gửi lại chỉ nhận `401`, không kích hoạt thu hồi hàng loạt — xem [12 · BE-25](12-danh-gia-va-de-xuat.md). Refresh thất bại (401/403) xoá luôn cookie ([12 · BE-24](12-danh-gia-va-de-xuat.md)). ⬜ Còn mở: refresh đồng thời từ nhiều tab có thể bị báo nhầm — [12 · BE-26](12-danh-gia-va-de-xuat.md) |
...
| SĐT giả khi đặt hàng (guest checkout, COD) | ✅ Admin xác minh qua cuộc gọi điện thoại, hệ thống ghi nhận + CHẶN THẬT ở backend: chuyển đơn sang "Đã xác nhận" đòi hỏi đã ghi nhận `confirmed: true` (`409 ORDER_CALL_NOT_CONFIRMED` nếu chưa) — xem [modules/domain-orders.md §11](modules/domain-orders.md) |
| Xác thực email trước khi thanh toán ONLINE | ⬜ Chưa cần — hiện chỉ COD (thanh toán online còn chờ khách chốt cổng, xem CHECKLIST.md Q1); OTP SMS tự động cũng cố ý chưa làm (cần dịch vụ SMS gateway trả phí bên ngoài) |
